...
## [2026-03-13] UI 최적화 및 위치 기반 동적 인사말 시스템 구축

### 🎨 UI/UX 개선
- **상단 공간 최적화**: `SumpyoAppBar` 높이(`120->100`) 및 타이틀 패딩 제거를 통해 화면 상단 여백을 컴팩트하게 개선.
- **웰니스 히어로 섹션 개선**: 높이 조정(`340->300`) 및 `BorderRadius.circular(48)` 적용으로 카드 상단 각진 문제 해결.
- **배경 융합 효과**: 히어로 섹션 테두리에 부드러운 블러(Soft Glow) 효과를 추가하여 배경과 자연스럽게 어우러지도록 수정.

### 🌍 위치 기반 동적 시스템
- **일출/일몰 API 연동**: `geolocator`와 `Sunrise-Sunset.org` API를 활용하여 실제 위치 기반 일출/일몰 데이터 획득.
- **4단계 인사말 세분화**: 새벽-아침-점심-저녁으로 인사말 로직 고도화.
  - 일출 전: 새벽
  - 일출 후 ~ 12시: 아침
  - 12시 ~ 일몰 전: 점심
  - 일몰 후: 저녁
- **정밀 시간 비교**: `DateTime.isBefore`를 사용한 분 단위 정밀 로직 구현 및 글로벌 타임존 대응 진단 로그 추가.

### ⚡ 상태 관리 및 안정성
- **깜빡임 제거(Warm Start)**: 주요 Provider(`Wellness`, `Prescription`)에 `keepAlive: true` 및 `AsyncValue` 라이프사이클을 적용하여 탭 이동 시 데이터가 끊김 없이 즉시 표시되도록 최적화.
- **상호작용 오류 수정**: 웰니스 미션 아이템에서 불필요한 날짜 검증 로직을 제거하여 클릭 및 완료 처리가 불가능하던 문제 해결.
- **신뢰성 검증**: `GreetingUtils`에 대한 포괄적인 유닛 테스트(`test/core/utils/greeting_utils_test.dart`) 추가 및 통과.
