...
- [Android] AGP 8.3.x 이하 환경에서 최신 androidx.core 라이브러리(1.16.0+)를 사용할 경우 발생하는 빌드 에러. 임시 방편으로 버전을 고정하기보다 AGP 8.6+ 업그레이드를 권장함.

### 2026-03-13: win32 환경에서의 쉘 명령어 및 도구 미비 이슈
- **실수 (The Problem)**: 윈도우 환경에서 `run_shell_command`를 통해 `&&` 연산자나 `sed` 명령어를 사용하여 파일을 수정하려 함. 또한 세션에 제공되지 않은 `write_file` 도구를 호출함.
- **결과**: `ParserError`로 인해 명령어가 실행되지 않거나 도구 미찾음 에러가 발생하여 작업이 중단됨.
- **방지**: 
  1. 윈도우 환경에서는 PowerShell 문법(`;` 연산자, `Set-Content` 등)을 명시적으로 사용하라.
  2. 도구 호출 전 반드시 현재 세션에서 사용 가능한 `available_tools` 목록을 재확인하라.
  3. 멀티라인 코드를 파일에 쓸 때는 PowerShell의 Here-String(`@'...'@`)을 사용하여 이스케이프 문제를 원천 차단하라.

### 2026-03-13: StringUtils.keepAll()로 인한 테스트 매칭 실패
- **실수 (The Problem)**: `StringUtils.keepAll()`이 적용된 문자열을 `contains()`나 `expect()`로 직접 비교함.
- **결과**: UI에는 정상적으로 보이지만, 문자열 사이에 제로 너비 조이너(`\u200D`)가 삽입되어 테스트에서 불일치로 판정됨.
- **방지**: 테스트 환경에서는 `text.replaceAll('\u200D', '')`를 통해 정규화된 텍스트를 비교하라.
