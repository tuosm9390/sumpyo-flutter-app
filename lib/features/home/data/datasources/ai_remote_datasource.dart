...
class AIRemoteDataSource {
  dynamic _model; // Changed from GenerativeModel? to dynamic to allow mocking of final class
  ...
  void _initModel() {
    try {
      ...
      if (apiKey == null || apiKey.isEmpty) {
        debugPrint('Warning: GEMINI_API_KEY is not defined or dotenv not loaded');
        return;
      }
...
''';
  ...
  Map<String, dynamic> _getFallbackResponse() {
    return {
      "title": "잠시 쉬어가는 시간의 약",
      "content": "현재 약방에 작은 문제가 생겨 처방전을 바로 조제하지 못했습니다. 하지만 기억해주세요. 당신이 느끼는 그 감정은 지극히 자연스러운 것이며, 잠시 멈추고 숨을 고르는 것만으로도 훌륭한 치유가 됩니다. 너무 애쓰지 않아도 괜찮습니다.",
